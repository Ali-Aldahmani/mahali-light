...
import SetupWizardStep from '@/components/setup/SetupWizardStep';
...
export default function SetupWizardPage() {
    ...
    return (
      ...
      >
        <div className="space-y-3">
          <Input label="Store name" value={store.store_name} onChange={(e: any) => setStore({ ...store, store_name: e.target.value })} required />
          <Input label="Store name (Arabic)" value={store.store_name_ar} onChange={(e: any) => setStore({ ...store, store_name_ar: e.target.value })} />
          <Textarea label="Address" value={store.store_address} onChange={(e: any) => setStore({ ...store, store_address: e.target.value })} rows={2} />
          <Input label="Phone" value={store.store_phone} onChange={(e: any) => setStore({ ...store, store_phone: e.target.value })} />
          <Input label="Email" value={store.store_email} onChange={(e: any) => setStore({ ...store, store_email: e.target.value })} />
          <Input label="TRN" value={store.store_trn} onChange={(e: any) => setStore({ ...store, store_trn: e.target.value })} />
  ...
  if (step === 3) {
    return (
      <SetupWizardStep step={3} total={TOTAL} title="VAT settings" onBack={() => setStep(2)} onNext={() => setStep(4)}>
        ...
        <Input label="VAT rate (%)" type="number" value={vat.vat_rate} onChange={(e: any) => setVat({ ...vat, vat_rate: e.target.value })} />
        <Input label="VAT number" value={vat.vat_number} onChange={(e: any) => setVat({ ...vat, vat_number: e.target.value })} />
    ...
    return (
      ...
      >
        <div className="space-y-3">
          ...
          {network.mode === 'client' && (
            <>
              <Input label="Server IP" value={network.server_ip} onChange={(e: any) => { setConnOk(false); setNetwork({ ...network, server_ip: e.target.value }); }} />
          ...
          <Input label="PC identifier" value={network.pc_identifier} onChange={(e: any) => setNetwork({ ...network, pc_identifier: e.target.value })} />
        </div>
      </SetupWizardStep>
    );
  }
  ...
  if (step === 5) {
    return (
      ...
      >
        <Input label="Full name" value={admin.full_name} onChange={(e: any) => setAdmin({ ...admin, full_name: e.target.value })} />
        <Input label="Username" value={admin.username} onChange={(e: any) => setAdmin({ ...admin, username: e.target.value })} />
        <Input label="Password" type="password" value={admin.password} onChange={(e: any) => setAdmin({ ...admin, password: e.target.value })} />
        <div className="h-1 rounded bg-surface-2">
          <div className="h-1 rounded bg-accent" style={{ width: `${(passwordStrength / 4) * 100}%` }} />
        </div>
        <Input label="Confirm password" type="password" value={admin.confirm} onChange={(e: any) => setAdmin({ ...admin, confirm: e.target.value })} />
      </SetupWizardStep>
    );
  }
  ...
  if (step === 6) {
    return (
      <SetupWizardStep step={6} total={TOTAL} title="Cash drawer" onBack={() => setStep(status?.has_admin ? 4 : 5)} onNext={() => setStep(7)}>
        ...
        <Input label="Opening balance (AED)" type="number" value={cash.opening_balance} onChange={(e: any) => setCash({ opening_balance: e.target.value })} />
      </SetupWizardStep>
    );
  }
  ...
  if (step === 7) {
    return (
      ...
      >
        ...
        {!bank.skip && (
          <>
            <Input label="Bank name" value={bank.bank_name} onChange={(e: any) => setBank({ ...bank, bank_name: e.target.value })} />
            <Input label="Account name" value={bank.account_name} onChange={(e: any) => setBank({ ...bank, account_name: e.target.value })} />
            <Input label="Account number" value={bank.account_number} onChange={(e: any) => setBank({ ...bank, account_number: e.target.value })} />
            <Input label="IBAN" value={bank.iban} onChange={(e: any) => setBank({ ...bank, iban: e.target.value })} />
          </>
        )}
      </SetupWizardStep>
    );
