...
const URGENCY_TONE: Record<string, string> = { blocking: 'error', major: 'warning', minor: 'success' };
              ...
              {detail.breadcrumbs?.length > 0 && (
                <section>
                  ...
                  <ol className="mt-2 max-h-40 space-y-1 overflow-auto text-xs text-ink-muted">
                    {detail.breadcrumbs.map((b: any, i: number) => (
                      <li key={i}>
...
function SummaryCard({ label, value, alert }: { label: string; value: any; alert?: boolean }) {
  return (
    <div
...
export default function BugReportsAdminPage() {
  return (
    <RequirePermission permission="bug.view_all">
      <BugReportsAdminPageInner />
    </RequirePermission>
  );
}
