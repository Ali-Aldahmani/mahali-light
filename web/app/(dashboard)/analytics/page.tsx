...
import CustomerLeaderboard from '@/components/analytics/CustomerLeaderboard';
...
import ReorderTable from '@/components/analytics/ReorderTable';
import AnnualPlanTable from '@/components/analytics/AnnualPlanTable';
import {
  BarChart,
  Bar,
  XAxis,
...
// Tab: Overview
// =======================================================================
function OverviewTab({ range }: { range: { start_date: string; end_date: string } }) {
  const [kpis, setKpis] = useState<any>(null);
  const [trends, setTrends] = useState<any[]>([]);
  const [groupBy, setGroupBy] = useState('month');

  useEffect(() => {
    getKPIs(range).then((r: any) => setKpis(r?.data || r)).catch(() => {});
  }, [range]);

  useEffect(() => {
    getNetProfitTrends({ ...range, group_by: groupBy })
      .then((r: any) => setTrends(r?.data || r))
      .catch(() => setTrends([]));
  }, [range, groupBy]);
...
// Tab: Products
// =======================================================================
function ProductsTab({ range }: { range: { start_date: string; end_date: string } }) {
  ...
  useEffect(() => {
    ...
    searchProducts(productQuery, 20)
      .then((res: any) => {
        ...
        if (cancelled) return;
        setProductOptions(
          items.map((p: any) => ({ value: p.id, label: p.name, description: p.sku || '' })),
        );
      })
      .catch(() => {});
    return () => {
  ...
  useEffect(() => {
    ...
    getProductSeasonality(selectedProductId, { years: 2 })
      .then((r: any) => setSeasonality(r?.data || r))
              ...
              {seasonality.peak_months?.length > 0 && (
                <div className="mt-2 text-xs text-ink-muted">
                  ...
                  <strong className="text-accent">
                    {seasonality.peak_months.map((m: number) => MONTH[m - 1]).join(', ')}
                  </strong>
                  {seasonality.slow_months?.length > 0 && (
                    <>
                      {' '}· Slow months:{' '}
                      <strong className="text-ink">
                        {seasonality.slow_months.map((m: number) => MONTH[m - 1]).join(', ')}
                      </strong>
                    </>
                  )}
                </div>
...
// Tab: Suppliers
// =======================================================================
function SuppliersTab({ range }: { range: { start_date: string; end_date: string } }) {
  const [top, setTop] = useState<any[]>([]);
  const [worst, setWorst] = useState<any[]>([]);
  useEffect(() => {
    Promise.all([
...
// Tab: Customers
// =======================================================================
function CustomersTab({ range }: { range: { start_date: string; end_date: string } }) {
  const [top, setTop] = useState<any[]>([]);
...
// Tab: Employees
// =======================================================================
function EmployeesTab({ range }: { range: { start_date: string; end_date: string } }) {
  const [rows, setRows] = useState<any[]>([]);
  useEffect(() => {
    getEmployeePerformance(range)
      .then((r: any) => setRows(r?.data || r || []))
      .catch(() => setRows([]));
  }, [range]);
      ...
      <div className="rounded-card border border-border bg-surface p-4">
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={compareData} margin={{ top: 8, right: 24, left: 0, bottom: 48 }}>
            ...
            <Tooltip
              formatter={(v: any, n: any) => (n === 'Revenue' ? formatCurrency(v) : v)}
              contentStyle={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, fontSize: 12 }}
            />
...
// Tab: Peaks
// =======================================================================
function PeaksTab({ range }: { range: { start_date: string; end_date: string } }) {
  const [heatmap, setHeatmap] = useState<any>({ cells: [], max_count: 0 });
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-card border border-border bg-surface p-4">
        {months ? (
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={months.current.series.map((row: any, idx: number) => ({
            ...
            >
              ...
              <Bar dataKey="current" name={`${months.year}`} radius={[4, 4, 0, 0]}>
                {months.current.series.map((row: any, idx: number) => (
                  <Cell
                    key={idx}
...
function ForecastingTab() {
  ...
  function handleRecalculate() {
    ...
    recalculateForecasts()
      ...
      .catch((err: any) => toast.error(err?.message || 'Recalculation failed.'))
      .finally(() => setBusy(false));
  }

  function handleExport() {
    downloadAnnualPlanExcel({ year }).catch((err: any) =>
      toast.error(err?.message || 'Export failed.'),
    );
  }

  function handleDismiss(row: any) {
    dismissReorderRecommendation(row.id)
      .then(() => {
        toast.success('Reorder dismissed.');
        setReorder((prev) => prev.filter((r) => r.id !== row.id));
      })
      .catch((err: any) => toast.error(err?.message || 'Could not dismiss.'));
  }
