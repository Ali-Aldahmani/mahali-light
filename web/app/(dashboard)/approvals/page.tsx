...
import { getApprovalQueue } from '@/services/notificationService';
import { approveAdjustment, rejectAdjustment } from '@/services/adjustmentService';
import { approveCount, rejectCount } from '@/services/stockCountService';
import { approveReturnRequest, rejectReturnRequest } from '@/services/returnService';
import {
  approveEditRequest,
  rejectEditRequest,
      ...
      {loading && !data ? (
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          ...
          <Section
            ...
            onSeeAll={() => router.push('/invoices/edit-requests')}
          >
            {(data?.invoice_edits || []).map((r: any) => (
              <ActionRow
                key={r.id}
                busy={busy === `edit:${r.id}`}
                onApprove={() =>
          ...
          >
            {(data?.stock_adjustments || []).map((a: any) => (
              <ActionRow
                key={a.id}
                busy={busy === `adj:${a.id}`}
                onApprove={() =>
          ...
          <Section
            ...
            onSeeAll={() => router.push('/inventory/counts')}
          >
            {(data?.stock_counts || []).map((c: any) => (
              <ActionRow
                key={c.id}
                busy={busy === `cnt:${c.id}`}
          ...
          <Section
            ...
            onSeeAll={() => router.push('/attendance')}
          >
            {(data?.attendance_corrections || []).map((c: any) => (
              <ActionRow
                key={c.id}
                busy={busy === `corr:${c.id}`}
                onApprove={() =>
          ...
          <Section
            ...
            onSeeAll={() => router.push('/attendance?tab=leaves')}
          >
