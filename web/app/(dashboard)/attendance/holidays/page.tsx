...
import { addHoliday, listHolidays, removeHoliday } from '@/services/holidayService';
...
function CalendarView({ year, holidays }: { year: number; holidays: Holiday[] }) {
    ...
    <div>
      ...
      {upcoming.length > 0 && (
        <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
          {upcoming.map((h) => (
            <div key={h.id} className="rounded-card border border-border bg-surface p-3 shadow-card">
              ...
              {items.map((h) => (
                <tr key={h.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3 text-right">
                    {canEdit && (
                      <Button variant="ghost" size="sm" onClick={() => setDeleting(h)}>
      ...
      <AddHolidaySlideOver open={open} onClose={() => setOpen(false)} onSaved={refresh} />
