...
import LeaveBalanceBar from '@/components/ui/LeaveBalanceBar';
...
function EditEntitlementsSlideOver({ open, onClose, row, year, onSaved }: any) {
  const [annual, setAnnual] = useState<number | string>(0);
  const [sick, setSick] = useState<number | string>(0);
    ...
    >
      <div className="space-y-4">
        <Input
          ...
          max={60}
          value={annual}
          onChange={(e: any) => setAnnual(e.target.value)}
        />
        <Input
          ...
          max={60}
          value={sick}
          onChange={(e: any) => setSick(e.target.value)}
        />
        <Input
          ...
          max={30}
          value={carried}
          onChange={(e: any) => setCarried(e.target.value)}
...
function LeaveBalancesPageInner() {
...
function FragmentRow({ row, isOpen, onToggle, canEdit, onEdit }: any) {
  return (
    <>
...
export default function LeaveBalancesPage() {
  return (
    <RequirePermission permission="attendance.view_all">
      <LeaveBalancesPageInner />
    </RequirePermission>
  );
}
