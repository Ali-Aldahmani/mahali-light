...
import OutstandingBalanceCard from '@/components/ui/OutstandingBalanceCard';
...
function CustomerProfilePageContent() {
  ...
  useEffect(() => {
    return onCustomerBalanceUpdate((evt: any) => {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        ...
        <StatCard
          ...
          tone={(customer.creditBalance || 0) > 0 ? 'warning' : 'default'}
        />
        <StatCard
...
function CreditCardInfo({ customer }: { customer: CustomerProfile }) {
...
function InvoicesTab({ customerId }: { customerId: string }) {
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      ...
      render: (r: InvoiceRow) => r.itemCount,
    },
    {
      key: 'total',
      header: 'Total',
      align: 'right' as const,
      render: (r: InvoiceRow) => formatCurrency(r.total),
    },
    {
      key: 'paid',
      header: 'Paid',
      align: 'right' as const,
      render: (r: InvoiceRow) => formatCurrency(r.amountPaid),
    },
    {
      key: 'balance',
      header: 'Balance',
  ...
  return (
    <Table
      columns={columns}
      rows={rows}
      rowKey={(r: InvoiceRow) => r.id}
      onRowClick={(r: InvoiceRow) => router.push(`/invoices/${r.id}`)}
      empty={
        <EmptyState
...
function PaymentsTab({
  ...
  const [data, setData] = useState<{ rows: PaymentRow[]; total: number }>({
    rows: [],
    total: 0,
  });
  const [loading, setLoading] = useState(true);
  const [voidingId, setVoidingId] = useState<number | string | null>(null);
  ...
  async function handleVoid(p: PaymentRow) {
        ...
        columns={[
          {
            key: 'date',
            header: 'Date',
            render: (r: PaymentRow) => formatDate(r.paymentDate || r.createdAt),
          },
          {
            key: 'amount',
            header: 'Amount',
          ...
          },
          {
            key: 'method',
            header: 'Method',
            render: (r: PaymentRow) => <PaymentMethodIcon method={r.paymentMethod} />,
          },
          {
            key: 'employee',
            header: 'Collected by',
            render: (r: PaymentRow) => r.employeeUsername || '—',
          },
          {
            key: 'notes',
            header: 'Notes',
...
function ReturnsTab({ customerId }: { customerId: string }) {
  const [data, setData] = useState<ReturnsData>({ items: [], stats: undefined });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    getCustomerReturns(customerId)
      .then((r: ReturnsData) => setData(r || { items: [], stats: undefined }))
      .catch(() => setData({ items: [], stats: undefined }))
...
function WarrantiesTab({ customerId }: { customerId: string }) {
