...
import { type CustomerRecord } from '@/components/customers/CustomerFormSlideOver';
...
function OutstandingReceivablesPageContent() {
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      key: 'phone',
      header: 'Phone',
      render: (r: ReceivableRow) => r.phone || '—',
    },
    {
      key: 'creditBalance',
      header: 'Balance',
  ...
  return (
    <div className="space-y-5">
      ...
      <Table
        columns={columns}
        rows={sortedRows}
        rowKey={(r: ReceivableRow) => r.id}
        loading={loading}
        onRowClick={(r: ReceivableRow) => router.push(`/customers/${r.id}`)}
        empty={
          <EmptyState
...
export default function OutstandingReceivablesPage() {
  return (
    <RequirePermission permission="customer.view_balance">
      <OutstandingReceivablesPageContent />
    </RequirePermission>
  );
}
