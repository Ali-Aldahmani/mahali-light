...
import { listPeriods, closePeriod, getPeriodChecklist } from '@/services/financeService';
...
function PeriodsPageInner() {
  ...
  const [closing, setClosing] = useState<{ period: any; checklist: any[] } | null>(null);
  ...
  function refresh() {
    ...
    listPeriods()
      ...
      .catch((err: any) => setError(err?.message || 'Failed to load periods.'))
  ...
  async function startClose(period: any) {
    try {
...
function ClosePeriodModal({
  ...
  open: boolean;
  closing: { period: any; checklist: any[] } | null;
  onClose: () => void;
  onClosed?: () => void;
}) {
