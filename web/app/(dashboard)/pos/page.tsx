...
import InvoiceTotalsBlock from '@/components/ui/InvoiceTotalsBlock';
import SplitPaymentBuilder from '@/components/ui/SplitPaymentBuilder';
...
import { useSocketStore, onStockUpdate, onPrintRequest } from '@/store/socketStore';
import { printReceipt } from '@/services/printService';
import { toast } from '@/store/toastStore';
import { searchProducts } from '@/services/productService';
import { listCategoriesFlat } from '@/services/categoryService';
import {
  createInvoice,
  addInvoicePayment,
...
function POSPageContent() {
  ...
  // ---- Variant selector ------------------------------------------------
  const [variantSelector, setVariantSelector] = useState<any>(null);
  ...
  useEffect(() => {
    listCategoriesFlat()
      .then((rows: any) => setCategories(rows || []))
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    return onPrintRequest((payload: any) => {
  ...
  function handleProductClick(variant: any) {
  ...
  async function submitInvoice() {
    ...
    const missingSerial = cart.find(
      (it: any) => it.requiresSerial && (!it.serialNumber || !it.serialValid),
    );
    if (missingSerial) {
      toast.error(
  ...
  return (
    <div className="-mx-8 -my-6 h-[calc(100vh-3.5rem)] flex">
      ...
      <CartPanel
        ...
        onSerialChange={(variantId: any, value: any, status: any) =>
      ...
      <POSVariantSelector
        ...
        siblings={products}
        onAdd={(v: any, qty: any) => addToCart(v, qty)}
      />
      ...
      {confirmOpen && (
        <ConfirmPayModal
          ...
          payments={payments}
          onAddPayment={(method: any, amount: any) => addPayment(method, amount)}
...
function ProductBrowser({
  ...
  onProductClick: (variant: any) => void;
  isOffline: boolean;
}) {
...
function CartPanel({
  ...
  onQtyChange: (variantId: any, qty: any) => void;
  onDiscountChange: (variantId: any, patch: any) => void;
  onSerialChange: (variantId: any, value: any, status: any) => void;
  onRemove: (variantId: any) => void;
        ...
        {cart.length === 0 ? (
          ...
          cart.map((it) => (
            <div key={it.variantId} className="space-y-1.5">
              <CartItem
                item={it}
                onQtyChange={(q: any) => onQtyChange(it.variantId, q)}
                onDiscountChange={(d: any) => onDiscountChange(it.variantId, d)}
              ...
              {it.requiresSerial && (
                <SerialNumberInput
                  ...
                  required
                  onChange={(v: any, status: any) => onSerialChange(it.variantId, v, status)}
                  className="px-1"
                />
              )}
            </div>
...
function ConfirmPayModal({
  ...
  customer: any;
  payments: any[];
  onAddPayment: (method: any, amount: any) => void;
  onUpdatePayment: (id: any, patch: any) => void;
  onRemovePayment: (id: any) => void;
  submitting: boolean;
  onCancel: () => void;
  onConfirm: () => void;
}) {
