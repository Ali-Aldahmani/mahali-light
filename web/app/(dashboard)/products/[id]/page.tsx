...
import VariantMatrix from '@/components/ui/VariantMatrix';
import ImageUpload from '@/components/ui/ImageUpload';
import RequirePermission from '@/components/guards/RequirePermission';
import ProductEditSlideOver from '@/components/products/ProductEditSlideOver';
import VariantFormSlideOver from '@/components/products/VariantFormSlideOver';
import {
  deleteProductImage,
  getProduct,
  getProductHistory,
...
function ProductDetailPageContent() {
  ...
  const [confirmDelVariant, setConfirmDelVariant] = useState<any>(null);
  ...
  useEffect(() => {
    const off = onProductUpdate((payload: any) => {
      if (payload?.id === id) {
        fetch();
  ...
  const tabItems = useMemo(() => {
    const items = [{ value: 'overview', label: 'Overview', icon: <Package size={14} /> }];
  ...
  async function removeImage() {
    try {
      ...
      setProduct((p: any) => (p ? { ...p, imagePath: null } : p));
  ...
  const totalStock = variants.reduce((a: number, v: any) => a + Number(v.stockQty || 0), 0);
  const totalQuarantine = variants.reduce(
    (a: number, v: any) => a + Number(v.quarantineQty || 0),
    0,
  );
  const stockValue = canViewCost
    ? variants.reduce(
        (a: number, v: any) => a + Number(v.stockQty || 0) * Number(v.costPrice || 0),
        0,
      )
    : null;
    ...
    <div>
      ...
      {tab === 'variants' && product.hasVariants && (
        <VariantsTab
          ...
          onDelete={(v: any) => setConfirmDelVariant(v)}
        />
      )}
...
function ProductWarrantyStats({ productId }: { productId: number }) {
    ...
    <div>
      ...
      {stats.mostRecentReason && (
        <div className="mt-3 text-xs text-ink-muted">
          ...
          <div className={`line-clamp-2 ${claimRateTone}`}>{stats.mostRecentReason}</div>
        </div>
      )}
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <Stat label="Total in stock">
          <span className="text-2xl font-semibold text-ink">
            ...
            <span className="text-sm font-normal text-ink-muted">{product.unitLabel}</span>
          </span>
        </Stat>
        <Stat label="Quarantined">
          <span className="text-2xl font-semibold text-ink">
            {formatQty(totalQuarantine)}{' '}
            <span className="text-sm font-normal text-ink-muted">{product.unitLabel}</span>
          </span>
        </Stat>
...
function HistoryTab({ loading, entries }: { loading: boolean; entries: any[] }) {
  if (loading) {
    return (
  ...
  if (!entries.length) {
    return (
      <div className="card p-10 text-center text-ink-muted text-sm">No history yet.</div>
    );
  }
  return (
    <div className="card overflow-hidden">
              ...
              <div className="min-w-0 flex-1">
                ...
                {(e.oldValue || e.newValue) && (
                  <details className="mt-2 text-xs text-ink-muted">
                    <summary className="cursor-pointer hover:text-ink">Show diff</summary>
...
function toneForAction(action: string): 'muted' | 'success' | 'error' | 'accent' | 'warning' {
