...
import VariantsEditor, { type VariantDraft } from '@/components/products/VariantsEditor';
...
import { generateInternalBarcode } from '@/services/variantService';
...
const EMPTY_BASIC = {
  ...
  soldBy: 'piece',
  unitLabel: 'pcs',
  defaultWarrantyMonths: 0 as number | string,
  reorderThreshold: 0 as number | string,
  hasVariants: false,
...
const EMPTY_SIMPLE = {
  ...
  barcode: '',
  supplierBarcode: '',
  sellingPrice: 0 as number | string,
  costPrice: 0 as number | string,
  openingStock: 0 as number | string,
  reorderThreshold: '' as number | string,
};
...
function NewProductPageContent() {
  ...
  function setBasicField(key: keyof typeof EMPTY_BASIC, value: any) {
    setBasic((b) => {
  ...
  // Barcode scan-to-autofill. Works with any USB HID "keyboard wedge" scanner
  // (e.g. NETUM NT-M1) — no drivers needed, it just types the code + Enter
  // into whatever has keyboard focus, which useBarcodeListener detects.
  const [scanLookup, setScanLookup] = useState(false);
  const [scanResult, setScanResult] = useState<{
    code: string;
    status: 'internal' | 'external' | 'not_found';
    label: string;
  ...
  async function handleScan(code: string) {
    ...
    setManualCode('');
    setScanLookup(true);
    // Always capture the scanned code into the barcode field, even before
    // the lookup resolves, so it's never lost.
    setSimple((s) => ({ ...s, barcode: s.barcode || clean }));
    try {
      const result = await lookupProductBarcode(clean);
      if (result.source === 'internal' && result.internal) {
        setScanResult({
          code: clean,
          status: 'internal',
          label: `Already used by "${result.internal.product_name}" — pick a different barcode.`,
        });
        toast.warning(
          `Barcode ${clean} is already used by "${result.internal.product_name}".`,
        );
        return;
      }
      if (result.source === 'external' && result.external) {
        const ext = result.external;
        setBasic((b) => ({
          ...b,
          name: b.name || ext.name || b.name,
          brand: b.brand || ext.brand || b.brand,
        }));
        setSimple((s) => ({
          ...s,
          barcode: s.barcode || clean,
          supplierBarcode: s.supplierBarcode || clean,
        }));
        setScanResult({
          code: clean,
          status: 'external',
          label: ext.name
            ? `Found "${ext.name}"${ext.brand ? ` (${ext.brand})` : ''} — filled name/brand.`
            : 'Found in the global database.',
          imageUrl: ext.imageUrl,
        });
        toast.success(`Filled details for barcode ${clean} from the global database.`);
        return;
      }
      setScanResult({
        code: clean,
        status: 'not_found',
        label: 'Not found anywhere — enter the product details manually.',
      });
      toast.info(`Barcode ${clean} not found. Fill the product details manually.`);
    } catch (err: any) {
      ...
      for (let i = 0; i < variants.length; i++) {
        ...
        for (const a of requiredAttrs) {
          if (!v.attributeValueIds?.[a.attributeId]) {
            setPricingError(`Variant ${i + 1}: "${a.name}" is required.`);
            return false;
          }
        }
  ...
  return (
    <div>
      ...
      <div className="card mt-4 p-4 flex flex-col sm:flex-row sm:items-center gap-3">
        {scanResult?.imageUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={scanResult.imageUrl}
            alt=""
            className="h-9 w-9 shrink-0 rounded-lg object-cover bg-surface-2"
          />
        ) : (
          <div className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-accent-light text-accent">
            {scanLookup ? <Loader2 size={16} className="animate-spin" /> : <Barcode size={16} />}
          </div>
        )}
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-ink">
            Scan a barcode with your USB scanner to auto-fill this product
          </p>
          <p className="text-xs text-ink-muted truncate">
            {scanResult
              ? `${scanResult.code} — ${scanResult.label}`
              : 'Works anywhere on this page — no need to click into a field first.'}
          </p>
        </div>
        <form
        ...
        <div className="xl:col-span-2">
          ...
          {stepIdx === 2 && (
            <StepReview
              ...
              variants={variants}
              categoryAttributes={categoryAttributes}
              categoryName={tree && findInTree(tree, basic.categoryId)?.name}
            />
          )}
        </div>
...
function StepBasic({
  ...
  basic: typeof EMPTY_BASIC;
  setField: (key: keyof typeof EMPTY_BASIC, value: any) => void;
  tree: any[];
    ...
    <div className="card p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <CategoryTreeSelect
          ...
          onChange={(id: number | null) => setField('categoryId', id)}
          required
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Select
          label="Sold by"
          value={basic.soldBy}
          onChange={(v: string) => setField('soldBy', v)}
          options={SOLD_BY}
        />
        <Input
          label="Unit label"
      ...
      <ImageUpload
        label="Product image"
        value={null}
        onChange={(file: File | null) => setField('imageFile', file)}
...
function StepPricing({
  ...
  basic: typeof EMPTY_BASIC;
  setField: (key: keyof typeof EMPTY_BASIC, value: any) => void;
  simple: typeof EMPTY_SIMPLE;
  setSimple: React.Dispatch<React.SetStateAction<typeof EMPTY_SIMPLE>>;
  ...
  return (
    <div className="space-y-4">
      ...
      <Input
        ...
        onChange={(e) => setSimple((s) => ({ ...s, reorderThreshold: e.target.value }))}
                  ...
                  {variants.map((v, i) => (
                    <tr key={i} className="border-t border-border">
                      {categoryAttributes.map((a) => {
                        ...
                        const val = id
                          ? a.values.find((x: any) => x.id === id)?.value
                          : null;
                        return (
...
function findInTree(tree: any[], id: number | null): any {
  if (!id) return null;
  const stack = [...(tree || [])];
  while (stack.length) {
    const n = stack.pop();
