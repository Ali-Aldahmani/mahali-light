...
import ReturnLookupSearch from '@/components/returns/ReturnLookupSearch';
import RefundPreview from '@/components/returns/RefundPreview';
import PriceDiffCalculator from '@/components/returns/PriceDiffCalculator';
...
import type { Invoice, InvoiceItem, RefundPlanEntry, ReplacementPlanItem, SelectOption } from '../types';
...
function NewReturnRequestPageContent() {
  ...
  const [selectedItems, setSelectedItems] = useState<Record<string, SelectedItemEntry>>({});
  // map of invoiceItemId -> { qty, condition, serial }

  const [refundPlan, setRefundPlan] = useState<RefundPlanEntry[]>([]);
  const [replacementItems, setReplacementItems] = useState<ReplacementPlanItem[]>([]);
  ...
  function updateSelectedItem(itemId: string, patch: Partial<SelectedItemEntry>) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push(`/returns/requests/${res.id}`);
    } catch (err: any) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="mt-8 flex items-center justify-between">
        <Button
          variant="ghost"
          onClick={step === 0 ? () => router.push('/returns') : back}
...
function ItemsStep({
  ...
  noInvoice: boolean;
  selectedItems: Record<string, SelectedItemEntry>;
  setSelectedItems: React.Dispatch<React.SetStateAction<Record<string, SelectedItemEntry>>>;
  toggleItem: (id: string) => void;
  updateSelectedItem: (id: string, patch: Partial<SelectedItemEntry>) => void;
  returnType: string;
  setReturnType: (v: string) => void;
  reason: string;
            ...
            return (
              <tr key={it.id} className="border-t border-border align-top">
                ...
                <td className="py-2 pr-3 text-ink-muted">
                  ...
                  {it.committedReturnQty != null && it.committedReturnQty > 0 && (
      ...
      {rows.map(([key, row]) => (
        ...
        >
          ...
          <Select
            ...
            onChange={(v: string) => update(key, { condition: v })}
            options={[
              { value: 'good', label: 'Good' },
              { value: 'defective', label: 'Defective' },
              { value: 'damaged', label: 'Damaged' },
...
function RefundPlanStep({
  total,
  plan,
  setPlan,
}: {
  total: number;
  plan: RefundPlanEntry[];
  setPlan: React.Dispatch<React.SetStateAction<RefundPlanEntry[]>>;
}) {
  function updateRow(idx: number, patch: Partial<RefundPlanEntry>) {
        ...
        {plan.map((p, idx) => (
          ...
          >
            <Select
              ...
              value={p.method}
              onChange={(v: string) => updateRow(idx, { method: v })}
              options={[
            ...
            <Input
              ...
              value={p.notes || ''}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateRow(idx, { notes: e.target.value })}
            />
            <button
              type="button"
...
function ReplaceStep({
  ...
  returnedItems: BackendItem[];
  totalReturned: number;
  replacementItems: ReplacementPlanItem[];
  setReplacementItems: React.Dispatch<React.SetStateAction<ReplacementPlanItem[]>>;
  ...
  function update(id: string | undefined, patch: Partial<ReplacementPlanItem>) {
    setReplacementItems((prev) =>
...
function SupplierReturnPlanStep({ total }: { total: number }) {
  return (
    <div className="card p-5">
