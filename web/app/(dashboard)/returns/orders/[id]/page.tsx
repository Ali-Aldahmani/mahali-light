...
import StockActionBadge from '@/components/ui/StockActionBadge';
import RequirePermission from '@/components/guards/RequirePermission';
import { getReturnOrder } from '@/services/returnOrderService';
...
function ReturnOrderDetailPageContent() {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 text-sm pt-2 border-t border-border">
              {order.customerName && (
                <Field
                  label="Customer"
                  value={
                    <Link href={`/customers/${order.customerId}`} className="text-accent hover:underline">
                      {order.customerName}
                    </Link>
                  }
                />
              ...
              {order.supplierName && (
                <Field
                  label="Supplier"
                  value={
                    <Link href={`/suppliers/${order.supplierId}`} className="text-accent hover:underline">
                      {order.supplierName}
                    </Link>
                  }
                />
              ...
              {order.originalInvoiceId && (
                <Field
                  ...
                  value={
                    <Link href={`/invoices/${order.originalInvoiceId}`} className="font-mono text-accent hover:underline">
              ...
              {order.replacementInvoiceId && (
                <Field
                  ...
                  value={
                    <Link href={`/invoices/${order.replacementInvoiceId}`} className="font-mono text-accent hover:underline">
              ...
              {order.requestNumber && (
                <Field
                  label="From request"
                  value={
                    <Link href={`/returns/requests/${order.returnRequestId}`} className="font-mono text-accent hover:underline">
                      {order.requestNumber}
                    </Link>
                  }
                />
        ...
        <div className="card p-5">
          ...
          {(order.refundPayments || []).length === 0 ? (
            ...
            <ul className="space-y-2">
              {(order.refundPayments || []).map((p: any) => {
