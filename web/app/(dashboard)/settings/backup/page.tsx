...
import BackupJobRow from '@/components/backup/BackupJobRow';
import RetentionSlider from '@/components/backup/RetentionSlider';
import RestoreConfirmModal from '@/components/backup/RestoreConfirmModal';
...
function BackupSettingsPageInner() {
  ...
  const isRunning = useMemo(() => jobs?.some((j: any) => j.status === 'running'), [jobs]);
  ...
  const handleConfirmRestore = async (job: any) => {
    setRestoreJob(null);
...
function SettingsForm({
  ...
  nasTesting: boolean;
  onTestNas: () => void;
  onRetentionCleanup: () => void;
}) {
  const set = (patch: Record<string, any>) => setDraft((d: any) => ({ ...d, ...patch }));
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      ...
      <SectionCard title="Destinations" icon={<Server size={16} className="text-accent" />}>
        ...
        <Input
          ...
          onChange={(e: any) => set({ local_path: e.target.value })}
      ...
      <SectionCard title="Retention" icon={<Trash2 size={16} className="text-accent" />}>
        <RetentionSlider
          ...
          max={30}
          unit="days"
          onChange={(v: number) => set({ retention_6h_days: v })}
        />
        <RetentionSlider
          ...
          max={120}
          unit="days"
          onChange={(v: number) => set({ retention_nightly_days: v })}
        />
        <RetentionSlider
          ...
          max={52}
          unit="weeks"
          onChange={(v: number) => set({ retention_weekly_weeks: v })}
        />
        <RetentionSlider
