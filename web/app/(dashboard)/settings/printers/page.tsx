...
function PrintSettingsPageInner() {
  ...
  const [storeForm, setStoreForm] = useState<Record<string, any> | null>(null);
  const [savingStore, setSavingStore] = useState(false);
  const [uploadingLogo, setUploadingLogo] = useState(false);

  const [printers, setPrinters] = useState<any[]>([]);
  const [printerSettings, setPrinterSettings] = useState<Record<string, any> | null>(null);
  const [printersLoading, setPrintersLoading] = useState(true);

  useEffect(() => {
    setStoreLoading(true);
    getStoreSettings()
      .then((s: any) => setStoreForm(s))
      .catch((err: any) => toast.error(err?.message || 'Could not load settings.'))
  ...
  const handleStoreChange = (field: string, value: any) => {
    setStoreForm((prev) => ({ ...(prev || {}), [field]: value }));
  };
  const handleStoreNested = (group: string, field: string, value: any) => {
    setStoreForm((prev) => ({
      ...(prev || {}),
  ...
  const savePrintLocal = async (patch: Record<string, any>) => {
    const next = await saveLocalPrintSettings(patch);
    setPrinterSettings(next || { ...printerSettings, ...patch });
  };

  const testPrint = async (kind: 'receipt' | 'invoice') => {
      ...
      <section className="card p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            ...
            onChange={(e: any) => handleStoreChange('storeName', e.target.value)}
            disabled={!canEdit}
          />
          <Input
            label="TRN (Tax Registration Number)"
            value={storeForm.storeTRN || ''}
            onChange={(e: any) => handleStoreChange('storeTRN', e.target.value)}
            disabled={!canEdit}
          ...
          <Input
            ...
            onChange={(e: any) => handleStoreChange('storeAddress', e.target.value)}
            disabled={!canEdit}
          ...
          <Input
            ...
            onChange={(e: any) => handleStoreChange('storePhone', e.target.value)}
            disabled={!canEdit}
          />
          <Input
            label="Email"
            ...
            onChange={(e: any) => handleStoreChange('storeEmail', e.target.value)}
            disabled={!canEdit}
          />
          <Input
            label="Currency"
            value={storeForm.currency || 'AED'}
            onChange={(e: any) => handleStoreChange('currency', e.target.value)}
            disabled={!canEdit}
          />
          <Input
            label="VAT rate (%)"
