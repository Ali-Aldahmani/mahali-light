...
import WarrantyStatusCard from '@/components/ui/WarrantyStatusCard';
import RequirePermission from '@/components/guards/RequirePermission';
import { useDebouncedValue } from '@/hooks/useDebouncedValue';
import { lookupWarranties } from '@/services/warrantyService';
import RaiseClaimSlideOver from '@/components/warranties/RaiseClaimSlideOver';
import type { Warranty } from '@/components/warranties/types';
        ...
        <div className="space-y-4">
          {results.map((w) => (
            <WarrantyStatusCard
              key={w.id}
              warranty={w}
              onRaiseClaim={(warranty: Warranty) => setClaimFor(warranty)}
            />
          ))}
        </div>
      )}
      ...
      <RaiseClaimSlideOver
        ...
        onClose={(refresh) => {
          ...
          if (refresh && debounced) {
            const seq = ++seqRef.current;
            lookupWarranties(debounced).then((d: Warranty[]) => {
