...
import { listClaims } from '@/services/warrantyClaimService';
import { formatDate } from '@/lib/utils/format';
import { onWarrantyClaimEvent } from '@/store/socketStore';
import type { SelectOption, WarrantyClaim } from '@/components/warranties/types';
  ...
  const columns = [
    {
      key: 'claim',
      header: 'Claim',
      render: (row: WarrantyClaim) => <span className="font-mono text-xs">{row.claimNumber}</span>,
    },
    {
      key: 'warranty',
      header: 'Warranty',
      render: (row: WarrantyClaim) => <span className="font-mono text-xs">{row.warrantyNumber}</span>,
    },
    {
      key: 'product',
      header: 'Product',
      render: (row: WarrantyClaim) => (
        <span className="text-sm truncate max-w-[220px] block">{row.productName || '—'}</span>
      ),
    },
    {
      key: 'customer',
    ...
    },
    {
      key: 'date',
      header: 'Claim date',
      render: (row: WarrantyClaim) => formatDate(row.claimDate),
    },
    {
      key: 'issue',
      header: 'Issue',
      render: (row: WarrantyClaim) => (
        <span className="text-sm line-clamp-2 max-w-[280px] block">{row.issueDescription}</span>
      ),
    },
    {
      key: 'resolution',
    ...
    },
    {
      key: 'status',
      header: 'Status',
      render: (row: WarrantyClaim) => <ClaimStatusBadge status={row.status} size="sm" />,
    },
  ];
    ...
    <div>
      ...
      <div className="card p-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
          ...
          />
          <Select value={status} onChange={(v: string) => setStatus(v)} options={STATUS_OPTIONS} />
          <Select
            value={resolution}
      ...
      <Table
        ...
        onRowClick={(row: WarrantyClaim) => router.push(`/warranty-claims/${row.id}`)}
        pagination={
          meta
            ? {
                page: meta.page,
