...
import CorrectionRequestCard from '@/components/ui/CorrectionRequestCard';
import RequestCorrectionSlideOver from '@/components/attendance/RequestCorrectionSlideOver';
import {
  approveCorrection,
  listCorrections,
  rejectCorrection,
...
export default function CorrectionsTab() {
  ...
  const [myItems, setMyItems] = useState<Correction[]>([]);
  ...
  async function refresh() {
    ...
    try {
      ...
      if (user?.id) {
        ...
        setMyItems((mine?.data || []).filter((c: Correction) => c.requestedBy === user.id));
      }
    } catch (err: any) {
  ...
  async function onApprove(c: Correction) {
    setBusy(true);
    try {
