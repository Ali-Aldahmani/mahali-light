...
import RequestLeaveSlideOver from '@/components/attendance/RequestLeaveSlideOver';
import {
  approveLeave,
  cancelLeave,
  listLeaves,
...
export default function LeavesTab() {
  ...
  async function onApprove(leave: Leave) {
    setBusy(true);
    try {
  ...
  async function onCancel(leave: Leave) {
    setBusy(true);
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <Section
        ...
        render={(l: Leave) => (
          <LeaveRow key={l.id} leave={l} canApprove={false} onCancel={onCancel} isOwn />
        )}
      />
      ...
      <RequestLeaveSlideOver
        ...
        employeeId={canApprove ? null : (user as any)?.employeeId}
...
function Section({
  ...
  loading: boolean;
  empty: string;
  render: (l: Leave) => React.ReactNode;
}) {
  return (
    <section>
    ...
    <tr className="border-t border-border align-top">
      ...
      <td className="max-w-xs px-4 py-3 text-ink-muted">
        ...
        {leave.status === 'rejected' && leave.rejectionReason && (
          <p className="mt-1 text-xs text-error">Rejected: {leave.rejectionReason}</p>
        )}
      </td>
      <td className="px-4 py-3">
