...
import EmployeeAttendanceCard from '@/components/ui/EmployeeAttendanceCard';
import ManualOverrideSlideOver from '@/components/attendance/ManualOverrideSlideOver';
...
export default function TodayTab() {
  ...
  const [overrideEmp, setOverrideEmp] = useState<TodayEmployee | null>(null);
  ...
  const employees: TodayEmployee[] = today?.employees || [];
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {COUNTER_META.map((c) => (
            <div key={c.key} className={`rounded-card px-4 py-2 ${c.tone}`}>
              <p className="text-xs font-medium">{c.label}</p>
              <p className="text-xl font-semibold leading-tight">
                {(counters as Record<string, number>)[c.key] || 0}
              </p>
            </div>
          ))}
        </div>
