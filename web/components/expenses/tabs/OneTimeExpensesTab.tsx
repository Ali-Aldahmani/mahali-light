...
import MonthlyExpenseSummary from '@/components/ui/MonthlyExpenseSummary';
import { listExpenses, deleteExpense, getExpenseSummary } from '@/services/expenseService';
...
export default function OneTimeExpensesTab({
  refreshTick,
  onAdd,
  onMutated,
}: {
  ...
  async function onDelete(expense: any) {
    try {
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      ...
      header: 'Amount',
      align: 'right',
      render: (r: any) => <span className="font-medium">{aed(r.amount)}</span>,
    },
    {
      key: 'paymentMethod',
      header: 'Method',
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Tile
          ...
          hint={`${summary?.byCategory?.reduce((n: number, c: any) => n + c.count, 0) || 0} expense(s)`}
        />
        <Tile
          label="This year"
        ...
        <div className="rounded-card border border-border bg-surface p-4 shadow-soft">
          ...
          <ul className="mt-2 space-y-1 text-sm">
            ...
            {(!summary?.byCategory || !summary.byCategory.some((c: any) => c.total > 0)) && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div>
          <MonthlyExpenseSummary
            ...
            month={month}
            year={year}
            onChangeMonth={({ month: m, year: y }: { month: number; year: number }) => {
              setMonth(m);
              setYear(y);
            }}
          />
