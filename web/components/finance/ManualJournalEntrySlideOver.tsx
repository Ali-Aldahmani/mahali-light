...
export default function ManualJournalEntrySlideOver({
  open,
  onClose,
  onPosted,
}: {
  ...
  function updateLine(id: number, patch: Partial<ReturnType<typeof emptyLine>>) {
            ...
            {lines.map((l) => (
              <div key={l.id} className="rounded-md border border-border p-2 space-y-2">
                <AccountSelector
                  accounts={accounts}
                  value={l.accountId}
                  onChange={(id: string) => updateLine(l.id, { accountId: id })}
                  placeholder="Account…"
                />
                <div className="flex gap-2">
