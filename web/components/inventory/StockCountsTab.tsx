...
import { listCounts } from '@/services/stockCountService';
import { useAuthStore } from '@/store/authStore';
import { formatRelativeTime, formatCurrency } from '@/lib/utils/format';
import { onCountEvent, onStockUpdate } from '@/store/socketStore';
import StartStockCountModal from './StartStockCountModal';
...
export default function StockCountsTab() {
  ...
  useEffect(() => {
    ...
    const unsubB = onStockUpdate((e: any) => {
    ...
    },
    {
      ...
      render: (r: any) => (
        <Button
          ...
          onClick={() => router.push(`/inventory/counts/${r.id}`)}
        >
          Open
        </Button>
      ),
  ...
  return (
    <div className="space-y-5">
      {activeCount && (
        <div className="rounded-card border border-warning/40 bg-warning-light px-4 py-3 flex items-center justify-between gap-3">
          ...
          <Button
            ...
            size="sm"
            rightIcon={<ArrowRight className="h-4 w-4" />}
            onClick={() => router.push(`/inventory/counts/${activeCount.id}`)}
          >
            Open count
          </Button>
        </div>
      ...
      <StartStockCountModal
        ...
        onCreated={(count: any) => {
          fetchData();
          if (count?.id) router.push(`/inventory/counts/${count.id}`);
        }}
      />
    </div>
  );
