...
function toCsv(rows: any[], columns: { label: string; value: (r: any) => any }[]) {
...
export default function StockLevelsTab() {
  ...
  const [movementsVariant, setMovementsVariant] = useState<any>(null);
  ...
  const categoryOptions = useMemo(
    () => [
      ...
      ...(categoriesFlat || []).map((c: any) => ({
        value: c.id,
        label: (typeof c.path === 'string' ? c.path : c.path?.map?.((p: any) => p.name).join(' / ')) || c.name,
      })),
    ],
    [categoriesFlat],
  );
  ...
  const columns = useMemo(() => {
    ...
    return cols;
  }, [router, canSeeCost, canRequestAdj]);
