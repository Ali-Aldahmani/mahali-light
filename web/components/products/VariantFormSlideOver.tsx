...
const EMPTY = {
  attributeValueIds: {} as Record<number, number | undefined>,
  sku: '',
  barcode: '',
  supplierBarcode: '',
...
export default function VariantFormSlideOver({
  ...
  useEffect(() => {
    ...
    setImageFile(null);
    if (initialValue) {
      const attrMap: Record<number, number | undefined> = {};
  ...
  function set(key: keyof typeof EMPTY, value: any) {
    setForm((f) => ({ ...f, [key]: value }));
  }

  function setAttr(attributeId: number, valueId: number) {
    setForm((f) => ({
      ...f,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {attributes.map((a) => (
                <Select
                  ...
                  onChange={(v: number) => setAttr(a.attributeId, v)}
                  options={(a.values || []).map((v: any) => ({
                    value: v.id,
                    label: v.value,
                  }))}
