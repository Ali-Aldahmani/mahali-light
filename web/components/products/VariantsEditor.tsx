...
type AttributeValueOption = { id: number; value: string };
...
export type VariantDraft = {
  attributeValueIds: Record<number, number | undefined>;
  sku: string;
  barcode: string;
...
export default function VariantsEditor({
  ...
  function setField(idx: number, key: keyof VariantDraft, value: string) {
    setVariants((arr) => arr.map((v, i) => (i === idx ? { ...v, [key]: value } : v)));
  }

  function setAttr(idx: number, attributeId: number, valueId: number) {
    setVariants((arr) =>
      arr.map((v, i) =>
        i === idx
          ? {
  ...
  function applyBulkPrice() {
    ...
    setVariants((arr) => arr.map((v) => ({ ...v, sellingPrice: Number(bulkPrice) })));
    toast.success('Applied selling price to all variants.');
  }

  async function generateBarcode(idx: number) {
    try {
                  ...
                  <Select
                    ...
                    label={
                      <span>
                        ...
                        {a.isRequired && <span className="text-error ml-0.5">*</span>}
                      </span>
                    }
                    options={opts}
                    value={v.attributeValueIds?.[a.attributeId] || ''}
                    onChange={(val: number) => setAttr(idx, a.attributeId, val)}
                    placeholder="—"
                  />
                );
              })}
