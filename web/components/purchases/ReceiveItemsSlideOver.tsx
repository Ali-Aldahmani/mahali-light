...
import { receivePurchaseOrderItems } from '@/services/purchaseOrderService';
...
interface ReceiveItemsSlideOverProps {
  open: boolean;
  onClose: () => void;
  po: PurchaseOrder | null;
  onReceived?: (updated: any) => void;
}
...
export default function ReceiveItemsSlideOver({
  open,
  onClose,
  po,
  onReceived,
}: ReceiveItemsSlideOverProps) {
  const remaining = useMemo<ReceiveRow[]>(() => {
  ...
  function patch(idx: number, val: number) {
    setItems((prev) =>
      prev.map((it, i) =>
