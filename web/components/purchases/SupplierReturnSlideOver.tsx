...
interface SupplierReturnSlideOverProps {
  open: boolean;
  onClose: () => void;
...
export default function SupplierReturnSlideOver({
  open,
  onClose,
  po,
  onCreated,
}: SupplierReturnSlideOverProps) {
  ...
  function patch(idx: number, p: Partial<ReturnRow>) {
          ...
          <div className="space-y-2">
            {items.map((it, idx) => (
              ...
              >
                ...
                <Select
                  value={it.condition}
                  onChange={(v: string) => patch(idx, { condition: v })}
                  options={CONDITIONS}
                  searchable={false}
                  className="w-32"
                />
