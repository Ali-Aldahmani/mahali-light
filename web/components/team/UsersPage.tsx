...
import { OnlineBadge, RoleBadge, StatusBadge } from '@/components/ui/Badge';
import EmptyState from '@/components/ui/EmptyState';
import ConfirmDialog from '@/components/ui/ConfirmDialog';
import PermissionGate from '@/components/ui/PermissionGate';
import UserFormSlideOver from './UserFormSlideOver';
import { deactivateUser, forceLogout, listUsers } from '@/services/userService';
...
export default function UsersPage() {
  ...
  const [confirm, setConfirm] = useState<any>(null); // { type, user }
    ...
    () => [
      {
        ...
        accessor: (r: any) => r.employee?.name || r.username,
      },
      {
        key: 'role',
        header: 'Role',
        render: (row: any) => <RoleBadge role={row.role?.name} />,
        accessor: (r: any) => r.role?.name,
      },
      {
        key: 'status',
        header: 'Status',
          ...
          <div className="flex items-center justify-end gap-1">
            <PermissionGate permission="user.change_role">
              <Button
                ...
                onClick={() => router.push(`/team/users/${row.id}/permissions`)}
              >
                Permissions
              </Button>
            </PermissionGate>
