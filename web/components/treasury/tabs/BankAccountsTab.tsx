...
type ActionKind = 'deposit' | 'withdraw' | 'history' | null;
    ...
    <SlideOver open={open} onClose={onClose} title="Add bank account" width="md">
      <form onSubmit={submit} className="space-y-3">
        ...
        <Input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('accountName', e.target.value)}
          required
        />
        <div className="grid grid-cols-2 gap-3">
          <Input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('accountNumber', e.target.value)}
          />
          <Input
            label="Currency"
            value={form.currency}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('currency', e.target.value)}
          />
        </div>
        <Input
          label="IBAN"
        ...
        <Input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('openingBalance', e.target.value)}
        />
...
function BankMovementSlideOver({
  account,
  open,
  onClose,
  onDone,
...
function HistorySlideOver({
  ...
  useEffect(() => {
    ...
    listBankTransactions(account.id, { limit: 50 })
      .then((res: { data?: BankTransaction[] }) => setTransactions(res?.data || []))
