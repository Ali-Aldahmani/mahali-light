...
import CashTransactionTypeBadge from '@/components/ui/CashTransactionTypeBadge';
import TransactionDirectionBadge from '@/components/ui/TransactionDirectionBadge';
import DiscrepancyAlert from '@/components/ui/DiscrepancyAlert';
...
import ReconciliationSummary, { type ReconciliationData } from './ReconciliationSummary';
...
export default function CashDrawerTab({ actionParam = null }: CashDrawerTabProps) {
          ...
          columns={[
            {
              key: 'timestamp',
              header: 'Time',
              render: (r: CashTransaction) => formatDateTime(r.timestamp),
            },
            {
              key: 'transactionType',
              header: 'Type',
              render: (r: CashTransaction) => <CashTransactionTypeBadge type={r.transactionType} />,
            },
            {
              key: 'direction',
              header: 'In / Out',
              render: (r: CashTransaction) => (
            ...
            },
            {
              ...
              align: 'right',
              render: (r: CashTransaction) => formatCurrency(r.balanceAfter),
            },
            {
              key: 'referenceType',
              header: 'Reference',
            ...
            },
            {
              ...
              header: 'By',
              render: (r: CashTransaction) => r.employeeUsername || '—',
            },
          ]}
          rows={transactions}
          rowKey={(r: CashTransaction) => r.id}
          ...
          columns={[
            {
              key: 'opened_at',
              header: 'Opened',
              render: (r: CashSession) => formatDateTime(r.openedAt),
            },
            {
              key: 'opened_by_username',
              header: 'Opened by',
              render: (r: CashSession) => r.openedByUsername || '—',
            },
            {
              key: 'opening_balance',
              header: 'Opening',
              align: 'right',
              render: (r: CashSession) => formatCurrency(r.openingBalance),
            },
            {
              key: 'closing_balance',
              header: 'Closing',
              align: 'right',
              render: (r: CashSession) =>
    ...
    <SlideOver open={open} onClose={onClose} title="Open cash drawer" width="md">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Input
          ...
          min="0"
          value={opening}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setOpening(e.target.value)}
          required
          autoFocus
        />
        <Textarea
...
function CloseDrawerSlideOver({
  ...
  onClose: () => void;
  expected: number;
  onDone?: (payload: ReconciliationData) => void;
}) {
    ...
    <SlideOver open={open} onClose={onClose} title="Close cash drawer" width="md">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Input
          ...
          min="0"
          value={counted}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCounted(e.target.value)}
          required
          autoFocus
        />
