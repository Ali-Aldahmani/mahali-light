...
interface PosState {
  ...
  setSerial: (
    variantId: string | number,
    serial: string,
    opts?: { valid?: boolean; error?: string | null },
  ) => void;
  updateQty: (variantId: string | number, qty: number) => void;
  updateUnitPrice: (variantId: string | number, unitPrice: number) => void;
  setLineDiscount: (
    variantId: string | number,
    opts?: { percent?: number | null; amount?: number | null },
  ) => void;
  ...
  clearCart: () => void;
  enqueueOfflineInvoice: (payload: any) => OfflineQueueEntry;
  markOfflineSynced: (id: string, result: any) => void;
  markOfflineError: (id: string, error: any) => void;
}
...
function loadPcIdentifier(): string {
  try {
...
function computeTotals(state: Pick<PosState, 'cart' | 'invoiceDiscount' | 'taxRate' | 'payments'>): PosTotals {
