...
import { getClaimsSummary } from '@/services/warrantyClaimService';

interface WarrantyState {
  activeCount: number;
  expiringSoonCount: number;
  expiringThisMonthCount: number;
  expiredThisYearCount: number;
  claimedCount: number;
  openClaimsCount: number;
  inProgressClaimsCount: number;
  resolvedThisMonthClaims: number;
  rejectedClaimsCount: number;
  supplierPendingClaims: number;
  expiringWarranties: any[];
  refresh: () => Promise<void>;
  refreshExpiringList: (limit?: number) => Promise<void>;
  reset: () => void;
}
